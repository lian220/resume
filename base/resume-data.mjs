...
export const experiences = [
  {
    ...
    analyze: [
      '외부 IdP 종속 대신 <b>토큰을 자체 발급·통제</b>하기로 결정 (Implicit Grant 배제, OAuth 2.1 방향).',
      '권한은 stale·위조를 감수하더라도 <b>게이트웨이 중앙 집행 + 서버사이드 이중검증</b>이 맞다고 판단 (JWT 권한은 FE 분기 전용).',
      '권한 폭증을 막기 위해 <b>계층형 권한 모델</b>이 필요하다고 분석.',
    ],
    action: [
      'OAuth 2.1 PKCE(S256) → 자체 JWT + <b>Refresh Token Rotation</b> (구 토큰 재사용 시 세션 전체 무효화)',
      'RBAC <b>3단 모델(ROOT/FUNCTION/ENDPOINT) + 와일드카드 계층 매칭 + 객체레벨 인가</b> (수직 권한 상승 차단, OWASP API1/3)',
      '<b>API 게이트웨이 중앙 RBAC 집행</b> + Redis 분산 캐시 + 즉시 무효화 API',
    ],
    results: [
  ...
  },
  {
    ...
    analyze: [
      ...
      '중복 방지는 분산락 대신 <b>DB 멱등성(유니크 제약)</b>으로, 리워드는 자동 재시도의 이중 지급 위험을 인지.',
    ],
    action: [
      '토스 빌링키만 저장, <b>idempotency 키(memberId+billingDate) + 조건부 UPDATE</b>로 중복 결제 차단',
      'API/DB 트랜잭션 경계 분리로 외부 지연에도 일관성 유지, exponential backoff 재시도 → N회 실패 시 연체 전환',
      '추천 리워드 <b>D+8 멱등 지급(PENDING→GRANTED, 정확히 1회)</b> + 환불 제외, <b>실패 격리 + Slack 알림 + 수동 멱등 재처리</b>',
    ],
    results: [
  ...
  },
  {
    ...
    analyze: [
      '타입별 테이블 분리 대신 <b>공통 라이프사이클 공유 모델(STI)</b>로 결정 — 폴리모픽 목록 조회 최적.',
      '알림톡은 단순 retry 시 <b>dual-write 유실 위험</b>이 있다고 판단 → 트랜잭션 경계 분리 필요.',
    ],
    action: [
      '3종 컨텐츠를 <b>STI(단일 테이블 상속)</b>로 모델링, 본부향 CRUD + 임시저장/발행, 점주향 동의·확인 흐름',
      '카카오 알림톡 연동 + <b>Transactional Outbox</b>로 부분 실패 시에도 게시글 상태 일관성 유지',
  ...
  },
  {
    ...
    task: "비용·데이터 주권을 모두 만족하는 관측성 대안을 직접 선정 → 도입 → 확산까지 책임.",
    problem: "운영 장애를 로그 grep으로만 추적 → 디버깅 병목. 팀 관측성 표준 자체가 부재, SaaS(Datadog/New Relic)는 비용 + 사용자/매출 데이터 외부 반출 우려.",
    analyze: [
      'SaaS(호스트·볼륨 과금) vs OSS를 <b>비용·데이터 주권 기준으로 비교</b> 후 SigNoz 채택을 직접 설득.',
      '"관측성 다운 ≠ 서비스 다운"이라 <b>HA는 과투자</b>로 판단 — 단일 인스턴스가 합리적.',
    ],
    action: [
      '<b>SigNoz self-hosted (EC2 단독 + docker-compose)</b> 도입, EBS 스냅샷 백업 + ClickHouse TTL 보존',
    ...
    results: [
      ...
      { t: "SaaS 대비 연 N만원 절감, 디버깅 N시간 → M분 단축, 확산 N명/N개 서비스", ok: false },
    ],
